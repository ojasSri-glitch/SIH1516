...
# API Key Configured for Multilingual Speech & Mistral AI Services
SPEECH_API_KEY = os.environ.get("SPEECH_API_KEY", "")
MISTRAL_API_KEY = os.environ.get("MISTRAL_API_KEY", "WI867vwHWtpTgwFQL5Z8ytlRGx859TqA")
...
PINCODE_DATABASE = {
    ...
    "sector 14 rohini": {"pincode": "110085", "district": "North West Delhi", "city": "Delhi", "state": "Delhi"},
    ...
    "bandra west": {"pincode": "400050", "district": "Mumbai Suburban", "city": "Mumbai", "state": "Maharashtra"},
...
def lookup_pincode(location_str: str) -> dict:
    ...
    for key, data in PINCODE_DATABASE.items():
        if key == loc_clean or key in loc_clean or loc_clean in key:
            return {"matched_key": key, **data}
...
def reverse_geocode_coordinates(lat: float, lon: float) -> dict:
    """Reverse geocodes (lat, lon) to get Indian locality, city, state, and PIN code."""
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "JanSamvaadAI-SIH1516/1.0 (GovTech Platform)"})
        res = urllib.request.urlopen(req, context=ctx, timeout=4)
        if res.getcode() == 200:
            data = json.loads(res.read().decode("utf-8"))
            addr = data.get("address", {})
            postcode = addr.get("postcode", "")
            city = addr.get("city") or addr.get("town") or addr.get("county") or addr.get("state_district") or "New Delhi"
            state = addr.get("state", "Delhi")
            suburb = addr.get("suburb") or addr.get("neighbourhood") or addr.get("road") or addr.get("residential") or city
            
            clean_pin = re.sub(r'\D', '', str(postcode))
            if len(clean_pin) == 6:
                postcode = clean_pin
            else:
                # If OSM didn't return a 6-digit pin, lookup our PIN database for the suburb/city
                suburb_match = lookup_pincode(f"{suburb} {city}")
                if suburb_match:
                    postcode = suburb_match.get("pincode", "110001")

            formatted_addr = f"{suburb}, {city}, {state}"
            return {
                "success": True,
                "latitude": lat,
                "longitude": lon,
                "pincode": postcode or "110001",
                "city": city,
                "state": state,
                "locality": suburb,
                "address": formatted_addr,
                "display_name": data.get("display_name", formatted_addr)
            }
    except Exception as e:
        print(f"[Reverse Geo Warning] Nominatim failed: {e}")

    # 2. Try BigDataCloud free client reverse geocoding API
    try:
        url = f"https://api.bigdatacloud.net/data/reverse-geocode-client?latitude={lat}&longitude={lon}&localityLanguage=en"
        req = urllib.request.Request(url, headers={"User-Agent": "JanSamvaadAI/1.0"})
        res = urllib.request.urlopen(req, context=ctx, timeout=4)
        if res.getcode() == 200:
            data = json.loads(res.read().decode("utf-8"))
            postcode = data.get("postcode", "")
            city = data.get("city") or data.get("locality") or "Urban Center"
            state = data.get("principalSubdivision", "Delhi")
            locality = data.get("locality") or city
            clean_pin = re.sub(r'\D', '', str(postcode))
            if len(clean_pin) == 6:
                postcode = clean_pin
            return {
                ...
                "state": state,
                "locality": locality,
                "address": f"{locality}, {city}, {state}"
            }
    except Exception:
        pass

    # 3. Coordinate Nearest Match Fallback from PIN database
    known_centers = [
        {"city": "New Delhi", "pincode": "110085", "state": "Delhi", "locality": "Rohini / North West Delhi", "lat": 28.7041, "lon": 77.1025},
        {"city": "Central Delhi", "pincode": "110001", "state": "Delhi", "locality": "Connaught Place", "lat": 28.6304, "lon": 77.2177},
        {"city": "Mumbai", "pincode": "400050", "state": "Maharashtra", "locality": "Bandra / Western Suburbs", "lat": 19.0760, "lon": 72.8777},
        {"city": "Bengaluru", "pincode": "560034", "state": "Karnataka", "locality": "Koramangala", "lat": 12.9716, "lon": 77.5946},
        {"city": "Chennai", "pincode": "600040", "state": "Tamil Nadu", "locality": "Anna Nagar", "lat": 13.0827, "lon": 80.2707},
        {"city": "Hyderabad", "pincode": "500034", "state": "Telangana", "locality": "Banjara Hills", "lat": 17.3850, "lon": 78.4867},
        {"city": "Kolkata", "pincode": "700091", "state": "West Bengal", "locality": "Salt Lake Sector V", "lat": 22.5726, "lon": 88.3639},
        {"city": "Pune", "pincode": "411038", "state": "Maharashtra", "locality": "Kothrud", "lat": 18.5204, "lon": 73.8567},
        {"city": "Ahmedabad", "pincode": "380015", "state": "Gujarat", "locality": "Satellite / Vastrapur", "lat": 23.0225, "lon": 72.5714}
    ]
    best = min(known_centers, key=lambda c: (c["lat"] - lat)**2 + (c["lon"] - lon)**2)
    return {
        "success": True,
        "latitude": lat,
        "longitude": lon,
        ...
        "address": f"{best['locality']}, {best['city']}, {best['state']} - {best['pincode']}"
    }

def get_ip_location() -> dict:
    """Detects user approximate location via IP when GPS is denied or blocked."""
    try:
        ctx = ssl.create_default_context()
        ctx.check_hostname = False
        ctx.verify_mode = ssl.CERT_NONE
        req = urllib.request.Request("http://ip-api.com/json/?fields=status,country,regionName,city,zip,lat,lon,district", headers={"User-Agent": "JanSamvaadAI/1.0"})
        res = urllib.request.urlopen(req, context=ctx, timeout=3)
        if res.getcode() == 200:
            data = json.loads(res.read().decode("utf-8"))
            if data.get("status") == "success":
                pincode = str(data.get("zip", ""))
                city = data.get("city", "New Delhi")
                state = data.get("regionName", "Delhi")
                locality = data.get("district") or city
                clean_pin = re.sub(r'\D', '', pincode)
                if len(clean_pin) != 6:
                    # Lookup city in PIN database
                    city_match = lookup_pincode(city)
                    clean_pin = city_match.get("pincode", "110001") if city_match else "110001"
                return {
                    "success": True,
                    ...
                    "state": state,
                    "locality": locality,
                    "address": f"{locality}, {city}, {state} - {clean_pin}"
                }
    except Exception as e:
        print(f"[IP Geo Warning] {e}")
...
class IndicAIEngine:
    ...
    def call_mistral(self, messages: list, json_mode: bool = True, temperature: float = 0.2) -> dict:
        """Helper to invoke Mistral AI Chat Completions endpoint."""
        api_key = self.mistral_key or MISTRAL_API_KEY
        if not api_key:
            return None
        try:
            ctx = ssl.create_default_context()
    ...
    def extract_entities(self, text: str) -> dict:
        ...
        for pat in landmark_patterns:
            ...
            if lm_match:
                candidate_lm = lm_match.group(1).strip()
                if len(candidate_lm) > 2 and len(candidate_lm) < 40:
                    entities["landmark"] = candidate_lm
                    break

        # Location candidate
        stopwords = {
            "हमारे", "मेरे", "हमारे इलाके", "our", "our area", "my area", "this", "my", "the", "a", "an",
            "road", "main road", "street", "causing accidents", "causing", "accident", "accidents",
            "pothole", "potholes", "burst", "leak", "dirty water", "garbage", "trash", "waste"
        }
        problem_keywords = [
            "causing", "accident", "pothole", "burst", "leak", "overflow", "danger", "broken",
            "problem", "issue", "dirty", "smell", "garbage", "waste", "stink", "spark", "blackout",
            "not working", "fire", "supply", "electricity", "water", "drain", "sewer", "safai"
        ]

        location_patterns = [
            r'((?:सेक्टर\s+\d+|वार्ड\s+\d+|फेज\s+\d+|ब्लॉक\s+[A-Za-z0-9]+)[\sA-Za-z\u0900-\u0D7F0-9\-]*)',
            r'(?:in|at|sector|pocket|phase|ward|colony|nagar|enclave)\s+([A-Za-z0-9\s\-]+?)(?:,|\.|\s+since|\s+for|\s+from|\s+mein|\s+में|$)',
            r'([A-Za-z\u0900-\u0D7F0-9\s]{3,35}?)\s+(?:में|इलाके में|क्षेत्र में|వద్ద|இல்)'
        ]
        for pat in location_patterns:
            loc_match = re.search(pat, text, re.IGNORECASE)
            if loc_match:
                loc_cand = loc_match.group(1).strip()
                loc_lower = loc_cand.lower()
                is_problem = any(pk in loc_lower for pk in problem_keywords)
                if len(loc_cand) > 2 and len(loc_cand) < 40 and loc_lower not in stopwords and not is_problem:
                    entities["location_candidate"] = loc_cand
                    break

        # Filter out problem description matches from landmark
        if entities.get("landmark"):
            lm_lower = entities["landmark"].lower()
            if any(pk in lm_lower for pk in ["causing", "accident", "pothole", "broken", "dirty"]):
                entities["landmark"] = None

        # Geo Postal DB enrichment
        geo_info = lookup_pincode(text)
        if geo_info:
            if not entities["pincode"]:
                entities["pincode"] = geo_info.get("pincode")
            if not entities["location_candidate"] or entities["location_candidate"].lower() in stopwords:
                entities["location_candidate"] = f"{geo_info.get('matched_key', '').title()}, {geo_info.get('city', '')}"
    ...
    def generate_officer_resolution_draft(self, grievance: dict) -> dict:
        # 1. Attempt Mistral AI LLM Generation
...
Return ONLY valid JSON matching this schema:
{
  ...
  "official_notice_template": "Formal departmental work order / closure memo format"
        ...
        messages = [
            ...
            {"role": "user", "content": f"Generate resolution template and officer copilot draft for this grievance: {json.dumps(grievance, ensure_ascii=False)}"}
        ]
        mistral_res = self.call_mistral(messages, json_mode=True, temperature=0.2)
        if mistral_res and isinstance(mistral_res, dict) and "suggested_action_notes" in mistral_res:
            return mistral_res
        ...
        return {
            ...
            "preventive_recommendations": "Regular maintenance and site inspection scheduled.",
            "official_notice_template": f"Work Order #{token_id}: Rectification completed for {category} at {grievance.get('address', 'Site')}."
        }

    def generate_mistral_template(self, grievance: dict, template_type: str = "work_order") -> dict:
        prompt = f"Generate an official government '{template_type}' document template for grievance: {json.dumps(grievance, ensure_ascii=False)}"
        system_prompt = "You are an official government administrative documentation expert for JanSamvaad AI (SIH1516). Generate professional, standard departmental memos, work orders, inspection notices, or citizen letters. Return JSON with 'template_type', 'title', 'subject', 'body', 'department', and 'reference_id'."
        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": prompt}
        ]
        result = self.call_mistral(messages, json_mode=True)
        if result and isinstance(result, dict):
            if "template_type" not in result:
                result["template_type"] = template_type
            if "title" not in result:
                result["title"] = f"Official Departmental Memo - {grievance.get('category', 'Civic Grievance')}"
            return {"success": True, "template": result, "engine": f"Mistral AI ({self.mistral_model})"}

        # Fallback
        return {
            "success": True,
            "engine": "JanSamvaad Template Generator (Fallback)",
            "template": {
                "template_type": template_type,
                "title": f"Official Departmental Memo - {grievance.get('category', 'Civic Grievance')}",
                "subject": f"Notice regarding Grievance #{grievance.get('id', 'JAN-2026')}",
                "body": f"Department: {grievance.get('department_name', 'Nodal Dept')}\nLocation: {grievance.get('address', 'Site')}\nSummary: {grievance.get('original_text', '')}",
                "reference_id": grievance.get("id", "JAN-2026")
            }
        }
