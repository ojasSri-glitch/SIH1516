...
const OFFICER_CREDENTIALS = { username: "mukund", password: "1234", name: "Officer Mukund" };
...
function handleOfficerLogin(event) {
  ...
  const username = (userElem ? userElem.value : "").trim().toLowerCase();
  const password = (passElem ? passElem.value : "").trim();

  // Hide any previous error
  if (errorElem) errorElem.style.display = "none";

  if ((username === "mukund" || username === OFFICER_CREDENTIALS.username) && 
      (password === "1234" || password === OFFICER_CREDENTIALS.password)) {
    // ✅ Credentials correct — store session and show dashboard
    sessionStorage.setItem("officer_logged_in", "true");
    sessionStorage.setItem("officer_name", OFFICER_CREDENTIALS.name);
    showOfficerDashboard();
  } else {
    // ❌ Wrong credentials — show error message
    if (errorElem) {
      errorElem.textContent = "❌ Incorrect username or password. (Hint: mukund / 1234)";
...
function initOfficerPortal() {
  ...
  if (loginBtn) {
    loginBtn.onclick = handleOfficerLogin;
  }

  const userInput = document.getElementById("officer-username");
  if (userInput) {
    userInput.addEventListener("keydown", (e) => {
      if (e.key === "Enter") handleOfficerLogin(e);
    });
  }

  const passInput = document.getElementById("officer-password");
  if (passInput) {
...
function renderKanbanBoard(grievances) {
  ...
  // Update badge counters safely
  const elFiled = document.getElementById("count-filed");
  if (elFiled) elFiled.textContent = counts.filed;
  const elProgress = document.getElementById("count-progress");
  if (elProgress) elProgress.textContent = counts.progress;
  const elAction = document.getElementById("count-action");
  if (elAction) elAction.textContent = counts.action;
  const elResolved = document.getElementById("count-resolved");
  if (elResolved) elResolved.textContent = counts.resolved;
}
